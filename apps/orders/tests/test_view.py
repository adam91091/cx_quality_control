...
class MeasurementReportsViewTest(TestCase):
    ...
    def setUpTestData(cls) -> None:
        ...
        cls.form_data = {**cls.meas_data, **cls.meas_report_data, }

        cls.user = CxUserFactory.create()

    def test_new_get(self):
        self.view_client.login(username=self.user.username, password=PASSWORD)
        response = assert_response_get(test_case=self, url_name='orders:measurement-report-new', id=self.order_new.id,
                                       exp_status_code=200, exp_template='measurement_report_form.html')
        self.assertEqual(response.context['order'], self.order_new)

        self.assertTrue(isinstance(response.context['form'], MeasurementReportForm))
        self.assertTrue(isinstance(response.context['formset'], MeasurementFormSet))

    def test_new_post(self):
        self.view_client.login(username=self.user.username, password=PASSWORD)
        assert_response_post(test_case=self, url_name='orders:measurement-report-new', id=self.order_new.id,
                             exp_status_code=302, data=self.form_data)
        self.assertEqual(self.order_new.measurement_report.measurements.count(), self.measurement_report_count)

    def test_update_get(self):
        self.view_client.login(username=self.user.username, password=PASSWORD)
        response = assert_response_get(test_case=self, url_name='orders:measurement-report-update',
                                       id=self.order_update.id,
                                       exp_status_code=200, exp_template='measurement_report_form.html')
        self.assertEqual(response.context['order'], self.order_update)
        self.assertEqual(response.context['form'].instance, self.order_update.measurement_report)
        self.assertEqual(len(response.context['formset']), self.measurement_report_count)
    ...
    def test_update_post(self):
        ...
        assert_response_post(test_case=self, url_name='orders:measurement-report-update', exp_status_code=302,
