...
class SpecificationPdfRenderView(SingleObjectMixin, View, LoginRequiredMixin,
...
class SpecificationIssueView(SingleObjectMixin, FormView, LoginRequiredMixin,
                             PermissionRequiredMixin):
    ...
    def post(self, request, *args, **kwargs):
        ...
        clients = Client.objects.filter(client_sap_id=client_sap_id)
        return self.form_valid(form) if clients else self.form_invalid(form)
    ...
    def form_valid(self, form):
        ...
        specification_issued = SpecificationIssued(client=client,
                                                   product=self.object,
                                                   date_of_issue=date_of_issue)
        for field in self.object.specification._meta.get_fields():
            field_value = getattr(self.object.specification, field.name)
            setattr(specification_issued, field.name, field_value)

        specification_issued.save()
...
class SpecificationIssueDeleteView(SuccessMessageMixin, LoginRequiredMixin, DeleteView,
                                   PermissionRequiredMixin):
    """Delete client issued specification."""
    model = SpecificationIssued
    template_name = 'specification_issue_confirm_delete.html'
    login_url = 'users:user-login'
    permission_required = ('clients.delete_client', )
    success_message = VIEW_MSG['specification']['issue_delete']
    ...
    def delete(self, request, *args, **kwargs):
        ...
        messages.success(self.request, self.success_message)
        return HttpResponseRedirect(reverse_lazy('clients:client-detail', kwargs={'pk': client.id}))
